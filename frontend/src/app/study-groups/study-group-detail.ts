...
export class StudyGroupDetail implements OnInit {
    ...
    statusSummary(group: StudyGroupViewModel): string {
        if (group.archived) {
            return 'This study group is archived and no longer accepts membership changes.';
        }
        if (group.isFull) {
            return 'This study group is currently full.';
        }
        return 'This study group is active and open for participation.';
    }
            ...
            .subscribe({
                next: (members) => {
                    this.group.update((group) => {
                        ...
                        const participantUserIds = Array.from(new Set(members.map((member) => member.user_id)));
                        return {
                            ...group,
                            isJoined: true,
                            participantUserIds,
                            ...
                            isFull: participantUserIds.length >= group.maxMembers,
                        };
                    });
        ...
        this.service
            ...
            .subscribe({
                next: (members) => {
                    ...
                    this.actionMessage.set('Left in UI immediately. Backend leave route is unavailable, so this change is local.');
                },
                error: (error) => {
                    this.group.set(current);
                    this.actionMessage.set(this.service.toErrorMessage(error));
